...
func VecAddSubScalarToAVX(avxType AVXType, opType OpType, isWordOp, isOrderReversed bool) {
	switch avxType {
	case TypeAVX2:
		switch opType {
		case OpAdd:
		...
		case OpSub:
			if isWordOp {
				if isOrderReversed {
					TEXT("subScalarVecWordToAVX2", NOSPLIT, "func(vOut []uint64, c uint64, v []uint64)")
				} else {
					TEXT("subVecScalarWordToAVX2", NOSPLIT, "func(vOut []uint64, v []uint64, c uint64)")
				}
			} else {
				if isOrderReversed {
					TEXT("subScalarVecToAVX2", NOSPLIT, "func(vOut []uint64, c uint64, v []uint64, q uint64)")
				} else {
					TEXT("subVecScalarToAVX2", NOSPLIT, "func(vOut []uint64, v []uint64, c uint64, q uint64)")
				}
			}
		}
	case TypeAVX512:
		...
		case OpSub:
			if isWordOp {
				if isOrderReversed {
					TEXT("subScalarVecWordToAVX512", NOSPLIT, "func(vOut []uint64, c uint64, v []uint64)")
				} else {
					TEXT("subVecScalarWordToAVX512", NOSPLIT, "func(vOut []uint64, v []uint64, c uint64)")
				}
			} else {
				if isOrderReversed {
					TEXT("subScalarVecToAVX512", NOSPLIT, "func(vOut []uint64, c uint64, v []uint64, q uint64)")
				} else {
					TEXT("subVecScalarToAVX512", NOSPLIT, "func(vOut []uint64, v []uint64, c uint64, q uint64)")
				}
			}
		}
	}
	...
	c := VMM(avxType)
	if isOrderReversed {
		VPBROADCASTQ(NewParamAddr("c", 24), c)
	} else {
