...
func (enc *Encryptor) EncryptCustom(v Plaintext, modLen int, isNTT bool) *Ciphertext {
	ct := NewCiphertextCustom(enc.Parameters().Rank(), modLen, isNTT)
	enc.EncryptTo(ct, v, isNTT)
	return ct
}
...
func (enc *Encryptor) EncryptElement(eIn *rlwe.Element, isNTT bool) *Ciphertext {
	ct := NewCiphertextCustom(enc.Parameters().Rank(), eIn.BaseModLen(), isNTT)
