...
func (ne *NoiseEstimator) getAuxMod(ct0, ct1 *Ciphertext) (int, int, *num.Modulus) {
	...
	case heint.VarianceType:
		scale = math.Max(1, crt.GaussianTailCut*math.Sqrt(ct0.noise/ne.noise.RoundNoise()))
...
func (ne *NoiseEstimator) MulPlainMatrixTo(ctOut, ct *Ciphertext, mat *rlwe.PlainMatrix) {
	ctOut.noise = ne.noise.MulPlainMatrix(ct.noise, mat, ct.ModLen())
}
