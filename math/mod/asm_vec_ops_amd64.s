// Code generated by command: go run main.go -vec -out ../../math/mod/asm_vec_ops_amd64.s -stubs ../../math/mod/asm_vec_ops_stub_amd64.go -pkg=mod. DO NOT EDIT.

//go:build amd64 && !purego

#include "textflag.h"

DATA ONE<>+0(SB)/8, $0x0000000000000001
GLOBL ONE<>(SB), RODATA|NOPTR, $8

// func addVecToAVX2(v0 []uint64, v1 []uint64, q uint64, vOut []uint64)
// Requires: AVX, AVX2, CMOV
TEXT ·addVecToAVX2(SB), NOSPLIT, $0-80
	MOVQ         q+48(FP), AX
	VPBROADCASTQ q+48(FP), Y0
	VPBROADCASTQ ONE<>+0(SB), Y1
	VPSUBQ       Y1, Y0, Y1
	MOVQ         v0_base+0(FP), CX
	MOVQ         v1_base+24(FP), DX
	MOVQ         vOut_base+56(FP), BX
	MOVQ         vOut_len+64(FP), SI
	MOVQ         SI, DI
	SHRQ         $0x02, DI
	SHLQ         $0x02, DI
	XORQ         R8, R8
	JMP          loop_end

loop_body:
	VMOVDQU  (CX)(R8*8), Y2
	VMOVDQU  (DX)(R8*8), Y3
	VPADDQ   Y3, Y2, Y2
	VPCMPGTQ Y1, Y2, Y3
	VPAND    Y0, Y3, Y3
	VPSUBQ   Y3, Y2, Y2
	VMOVDQU  Y2, (BX)(R8*8)
	ADDQ     $0x04, R8

loop_end:
	CMPQ R8, DI
	JL   loop_body
	JMP  leftover_loop_end

leftover_loop_body:
	MOVQ    (CX)(R8*8), DI
	MOVQ    (DX)(R8*8), R9
	ADDQ    R9, DI
	MOVQ    DI, R9
	SUBQ    AX, R9
	CMPQ    DI, AX
	CMOVQGE R9, DI
	MOVQ    DI, (BX)(R8*8)
	ADDQ    $0x01, R8

leftover_loop_end:
	CMPQ R8, SI
	JL   leftover_loop_body
	RET

// func addLazyVecToAVX2(v0 []uint64, v1 []uint64, vOut []uint64)
// Requires: AVX, AVX2
TEXT ·addLazyVecToAVX2(SB), NOSPLIT, $0-72
	MOVQ v0_base+0(FP), AX
	MOVQ v1_base+24(FP), CX
	MOVQ vOut_base+48(FP), DX
	MOVQ vOut_len+56(FP), BX
	MOVQ BX, SI
	SHRQ $0x02, SI
	SHLQ $0x02, SI
	XORQ DI, DI
	JMP  loop_end

loop_body:
	VMOVDQU (AX)(DI*8), Y0
	VMOVDQU (CX)(DI*8), Y1
	VPADDQ  Y1, Y0, Y0
	VMOVDQU Y0, (DX)(DI*8)
	ADDQ    $0x04, DI

loop_end:
	CMPQ DI, SI
	JL   loop_body
	JMP  leftover_loop_end

leftover_loop_body:
	MOVQ (AX)(DI*8), SI
	MOVQ (CX)(DI*8), R8
	ADDQ R8, SI
	MOVQ SI, (DX)(DI*8)
	ADDQ $0x01, DI

leftover_loop_end:
	CMPQ DI, BX
	JL   leftover_loop_body
	RET

// func subVecToAVX2(v0 []uint64, v1 []uint64, q uint64, vOut []uint64)
// Requires: AVX, AVX2, CMOV
TEXT ·subVecToAVX2(SB), NOSPLIT, $0-80
	MOVQ         q+48(FP), AX
	VPBROADCASTQ q+48(FP), Y0
	MOVQ         v0_base+0(FP), CX
	MOVQ         v1_base+24(FP), DX
	MOVQ         vOut_base+56(FP), BX
	MOVQ         vOut_len+64(FP), SI
	MOVQ         SI, DI
	SHRQ         $0x02, DI
	SHLQ         $0x02, DI
	VPXOR        Y1, Y1, Y1
	XORQ         R8, R8
	JMP          loop_end

loop_body:
	VMOVDQU  (CX)(R8*8), Y2
	VMOVDQU  (DX)(R8*8), Y3
	VPSUBQ   Y3, Y2, Y2
	VPCMPGTQ Y2, Y1, Y3
	VPAND    Y0, Y3, Y3
	VPADDQ   Y3, Y2, Y2
	VMOVDQU  Y2, (BX)(R8*8)
	ADDQ     $0x04, R8

loop_end:
	CMPQ R8, DI
	JL   loop_body
	JMP  leftover_loop_end

leftover_loop_body:
	MOVQ    (CX)(R8*8), DI
	MOVQ    (DX)(R8*8), R9
	SUBQ    R9, DI
	MOVQ    DI, R9
	ADDQ    AX, R9
	CMPQ    DI, $0x00
	CMOVQLT R9, DI
	MOVQ    DI, (BX)(R8*8)
	ADDQ    $0x01, R8

leftover_loop_end:
	CMPQ R8, SI
	JL   leftover_loop_body
	RET

// func subLazyVecToAVX2(v0 []uint64, v1 []uint64, vOut []uint64)
// Requires: AVX, AVX2
TEXT ·subLazyVecToAVX2(SB), NOSPLIT, $0-72
	MOVQ  v0_base+0(FP), AX
	MOVQ  v1_base+24(FP), CX
	MOVQ  vOut_base+48(FP), DX
	MOVQ  vOut_len+56(FP), BX
	MOVQ  BX, SI
	SHRQ  $0x02, SI
	SHLQ  $0x02, SI
	VPXOR Y0, Y0, Y0
	XORQ  DI, DI
	JMP   loop_end

loop_body:
	VMOVDQU (AX)(DI*8), Y0
	VMOVDQU (CX)(DI*8), Y1
	VPSUBQ  Y1, Y0, Y0
	VMOVDQU Y0, (DX)(DI*8)
	ADDQ    $0x04, DI

loop_end:
	CMPQ DI, SI
	JL   loop_body
	JMP  leftover_loop_end

leftover_loop_body:
	MOVQ (AX)(DI*8), SI
	MOVQ (CX)(DI*8), R8
	SUBQ R8, SI
	MOVQ SI, (DX)(DI*8)
	ADDQ $0x01, DI

leftover_loop_end:
	CMPQ DI, BX
	JL   leftover_loop_body
	RET

// func scalarMulVecToX86(v0 []uint64, c uint64, q uint64, divHi uint64, divLo uint64, vOut []uint64)
// Requires: BMI2, CMOV
TEXT ·scalarMulVecToX86(SB), NOSPLIT, $0-80
	MOVQ q+32(FP), AX
	MOVQ divHi+40(FP), CX
	MOVQ divLo+48(FP), BX
	MOVQ c+24(FP), SI
	MOVQ v0_base+0(FP), DI
	MOVQ vOut_base+56(FP), R8
	MOVQ vOut_len+64(FP), R9
	XORQ R10, R10
	JMP  loop_end

loop_body:
	MOVQ    (DI)(R10*8), DX
	MULXQ   SI, R12, R11
	MOVQ    R11, R13
	IMULQ   CX, R13
	MOVQ    R12, DX
	MULXQ   CX, R15, R14
	ADDQ    R14, R13
	MULXQ   BX, DX, R14
	MOVQ    R11, DX
	MULXQ   BX, R11, DX
	ADDQ    DX, R13
	ADDQ    R15, R14
	ADCQ    $0x00, R13
	ADDQ    R11, R14
	ADCQ    $0x00, R13
	IMULQ   AX, R13
	MOVQ    R12, DX
	SUBQ    R13, DX
	MOVQ    DX, R11
	SUBQ    AX, R11
	CMPQ    DX, AX
	CMOVQGE R11, DX
	MOVQ    DX, (R8)(R10*8)
	ADDQ    $0x01, R10

loop_end:
	CMPQ R10, R9
	JL   loop_body
	RET

// func scalarMulVecAddToX86(v0 []uint64, c uint64, q uint64, divHi uint64, divLo uint64, vOut []uint64)
// Requires: BMI2, CMOV
TEXT ·scalarMulVecAddToX86(SB), NOSPLIT, $8-80
	MOVQ q+32(FP), AX
	MOVQ divHi+40(FP), CX
	MOVQ divLo+48(FP), BX
	MOVQ c+24(FP), SI
	MOVQ v0_base+0(FP), DI
	MOVQ vOut_base+56(FP), R8
	MOVQ vOut_len+64(FP), R9
	XORQ R10, R10
	JMP  loop_end

loop_body:
	MOVQ    (DI)(R10*8), DX
	MOVQ    (R8)(R10*8), R11
	MULXQ   SI, R13, R12
	MOVQ    R12, R14
	IMULQ   CX, R14
	MOVQ    R13, DX
	MULXQ   CX, BP, R15
	ADDQ    R15, R14
	MULXQ   BX, DX, R15
	MOVQ    R12, DX
	MULXQ   BX, R12, DX
	ADDQ    DX, R14
	ADDQ    BP, R15
	ADCQ    $0x00, R14
	ADDQ    R12, R15
	ADCQ    $0x00, R14
	IMULQ   AX, R14
	MOVQ    R13, DX
	SUBQ    R14, DX
	MOVQ    DX, R12
	SUBQ    AX, R12
	CMPQ    DX, AX
	CMOVQGE R12, DX
	ADDQ    DX, R11
	MOVQ    R11, DX
	SUBQ    AX, DX
	CMPQ    R11, AX
	CMOVQGE DX, R11
	MOVQ    R11, (R8)(R10*8)
	ADDQ    $0x01, R10

loop_end:
	CMPQ R10, R9
	JL   loop_body
	RET

// func scalarMulVecSubToX86(v0 []uint64, c uint64, q uint64, divHi uint64, divLo uint64, vOut []uint64)
// Requires: BMI2, CMOV
TEXT ·scalarMulVecSubToX86(SB), NOSPLIT, $8-80
	MOVQ q+32(FP), AX
	MOVQ divHi+40(FP), CX
	MOVQ divLo+48(FP), BX
	MOVQ c+24(FP), SI
	MOVQ v0_base+0(FP), DI
	MOVQ vOut_base+56(FP), R8
	MOVQ vOut_len+64(FP), R9
	XORQ R10, R10
	JMP  loop_end

loop_body:
	MOVQ    (DI)(R10*8), DX
	MOVQ    (R8)(R10*8), R11
	MULXQ   SI, R13, R12
	MOVQ    R12, R14
	IMULQ   CX, R14
	MOVQ    R13, DX
	MULXQ   CX, BP, R15
	ADDQ    R15, R14
	MULXQ   BX, DX, R15
	MOVQ    R12, DX
	MULXQ   BX, R12, DX
	ADDQ    DX, R14
	ADDQ    BP, R15
	ADCQ    $0x00, R14
	ADDQ    R12, R15
	ADCQ    $0x00, R14
	IMULQ   AX, R14
	MOVQ    R13, DX
	SUBQ    R14, DX
	MOVQ    DX, R12
	SUBQ    AX, R12
	CMPQ    DX, AX
	CMOVQGE R12, DX
	SUBQ    DX, R11
	MOVQ    R11, DX
	ADDQ    AX, DX
	CMPQ    R11, $0x00
	CMOVQLT DX, R11
	MOVQ    R11, (R8)(R10*8)
	ADDQ    $0x01, R10

loop_end:
	CMPQ R10, R9
	JL   loop_body
	RET

// func scalarMulLazyVecToX86(v0 []uint64, c uint64, q uint64, divHi uint64, divLo uint64, vOut []uint64)
// Requires: BMI2
TEXT ·scalarMulLazyVecToX86(SB), NOSPLIT, $0-80
	MOVQ q+32(FP), AX
	MOVQ divHi+40(FP), CX
	MOVQ divLo+48(FP), BX
	MOVQ c+24(FP), SI
	MOVQ v0_base+0(FP), DI
	MOVQ vOut_base+56(FP), R8
	MOVQ vOut_len+64(FP), R9
	XORQ R10, R10
	JMP  loop_end

loop_body:
	MOVQ  (DI)(R10*8), DX
	MULXQ SI, R12, R11
	MOVQ  R11, R13
	IMULQ CX, R13
	MOVQ  R12, DX
	MULXQ CX, R15, R14
	ADDQ  R14, R13
	MULXQ BX, DX, R14
	MOVQ  R11, DX
	MULXQ BX, R11, DX
	ADDQ  DX, R13
	ADDQ  R15, R14
	ADCQ  $0x00, R13
	ADDQ  R11, R14
	ADCQ  $0x00, R13
	IMULQ AX, R13
	MOVQ  R12, DX
	SUBQ  R13, DX
	MOVQ  DX, (R8)(R10*8)
	ADDQ  $0x01, R10

loop_end:
	CMPQ R10, R9
	JL   loop_body
	RET

// func scalarMulAddLazyVecToX86(v0 []uint64, c uint64, q uint64, divHi uint64, divLo uint64, vOut []uint64)
// Requires: BMI2
TEXT ·scalarMulAddLazyVecToX86(SB), NOSPLIT, $8-80
	MOVQ q+32(FP), AX
	MOVQ divHi+40(FP), CX
	MOVQ divLo+48(FP), BX
	MOVQ c+24(FP), SI
	MOVQ v0_base+0(FP), DI
	MOVQ vOut_base+56(FP), R8
	MOVQ vOut_len+64(FP), R9
	XORQ R10, R10
	JMP  loop_end

loop_body:
	MOVQ  (DI)(R10*8), DX
	MOVQ  (R8)(R10*8), R11
	MULXQ SI, R13, R12
	MOVQ  R12, R14
	IMULQ CX, R14
	MOVQ  R13, DX
	MULXQ CX, BP, R15
	ADDQ  R15, R14
	MULXQ BX, DX, R15
	MOVQ  R12, DX
	MULXQ BX, R12, DX
	ADDQ  DX, R14
	ADDQ  BP, R15
	ADCQ  $0x00, R14
	ADDQ  R12, R15
	ADCQ  $0x00, R14
	IMULQ AX, R14
	MOVQ  R13, DX
	SUBQ  R14, DX
	ADDQ  DX, R11
	MOVQ  R11, (R8)(R10*8)
	ADDQ  $0x01, R10

loop_end:
	CMPQ R10, R9
	JL   loop_body
	RET

// func scalarMulSubLazyVecToX86(v0 []uint64, c uint64, q uint64, divHi uint64, divLo uint64, vOut []uint64)
// Requires: BMI2
TEXT ·scalarMulSubLazyVecToX86(SB), NOSPLIT, $8-80
	MOVQ q+32(FP), AX
	MOVQ divHi+40(FP), CX
	MOVQ divLo+48(FP), BX
	MOVQ c+24(FP), SI
	NEGQ SI
	ADDQ AX, SI
	MOVQ v0_base+0(FP), DI
	MOVQ vOut_base+56(FP), R8
	MOVQ vOut_len+64(FP), R9
	XORQ R10, R10
	JMP  loop_end

loop_body:
	MOVQ  (DI)(R10*8), DX
	MOVQ  (R8)(R10*8), R11
	MULXQ SI, R13, R12
	MOVQ  R12, R14
	IMULQ CX, R14
	MOVQ  R13, DX
	MULXQ CX, BP, R15
	ADDQ  R15, R14
	MULXQ BX, DX, R15
	MOVQ  R12, DX
	MULXQ BX, R12, DX
	ADDQ  DX, R14
	ADDQ  BP, R15
	ADCQ  $0x00, R14
	ADDQ  R12, R15
	ADCQ  $0x00, R14
	IMULQ AX, R14
	MOVQ  R13, DX
	SUBQ  R14, DX
	ADDQ  DX, R11
	MOVQ  R11, (R8)(R10*8)
	ADDQ  $0x01, R10

loop_end:
	CMPQ R10, R9
	JL   loop_body
	RET

// func mulVecToX86(v0 []uint64, v1 []uint64, q uint64, divHi uint64, divLo uint64, vOut []uint64)
// Requires: BMI2, CMOV
TEXT ·mulVecToX86(SB), NOSPLIT, $0-96
	MOVQ q+48(FP), AX
	MOVQ divHi+56(FP), CX
	MOVQ divLo+64(FP), BX
	MOVQ v0_base+0(FP), SI
	MOVQ v1_base+24(FP), DI
	MOVQ vOut_base+72(FP), R8
	MOVQ vOut_len+80(FP), R9
	XORQ R10, R10
	JMP  loop_end

loop_body:
	MOVQ    (SI)(R10*8), DX
	MOVQ    (DI)(R10*8), R11
	MULXQ   R11, R12, R11
	MOVQ    R11, R13
	IMULQ   CX, R13
	MOVQ    R12, DX
	MULXQ   CX, R15, R14
	ADDQ    R14, R13
	MULXQ   BX, DX, R14
	MOVQ    R11, DX
	MULXQ   BX, R11, DX
	ADDQ    DX, R13
	ADDQ    R15, R14
	ADCQ    $0x00, R13
	ADDQ    R11, R14
	ADCQ    $0x00, R13
	IMULQ   AX, R13
	MOVQ    R12, DX
	SUBQ    R13, DX
	MOVQ    DX, R11
	SUBQ    AX, R11
	CMPQ    DX, AX
	CMOVQGE R11, DX
	MOVQ    DX, (R8)(R10*8)
	ADDQ    $0x01, R10

loop_end:
	CMPQ R10, R9
	JL   loop_body
	RET

// func mulAddVecToX86(v0 []uint64, v1 []uint64, q uint64, divHi uint64, divLo uint64, vOut []uint64)
// Requires: BMI2, CMOV
TEXT ·mulAddVecToX86(SB), NOSPLIT, $8-96
	MOVQ q+48(FP), AX
	MOVQ divHi+56(FP), CX
	MOVQ divLo+64(FP), BX
	MOVQ v0_base+0(FP), SI
	MOVQ v1_base+24(FP), DI
	MOVQ vOut_base+72(FP), R8
	MOVQ vOut_len+80(FP), R9
	XORQ R10, R10
	JMP  loop_end

loop_body:
	MOVQ    (SI)(R10*8), DX
	MOVQ    (DI)(R10*8), R11
	MOVQ    (R8)(R10*8), R12
	MULXQ   R11, R13, R11
	MOVQ    R11, R14
	IMULQ   CX, R14
	MOVQ    R13, DX
	MULXQ   CX, BP, R15
	ADDQ    R15, R14
	MULXQ   BX, DX, R15
	MOVQ    R11, DX
	MULXQ   BX, R11, DX
	ADDQ    DX, R14
	ADDQ    BP, R15
	ADCQ    $0x00, R14
	ADDQ    R11, R15
	ADCQ    $0x00, R14
	IMULQ   AX, R14
	MOVQ    R13, DX
	SUBQ    R14, DX
	MOVQ    DX, R11
	SUBQ    AX, R11
	CMPQ    DX, AX
	CMOVQGE R11, DX
	ADDQ    DX, R12
	MOVQ    R12, DX
	SUBQ    AX, DX
	CMPQ    R12, AX
	CMOVQGE DX, R12
	MOVQ    R12, (R8)(R10*8)
	ADDQ    $0x01, R10

loop_end:
	CMPQ R10, R9
	JL   loop_body
	RET

// func mulSubVecToX86(v0 []uint64, v1 []uint64, q uint64, divHi uint64, divLo uint64, vOut []uint64)
// Requires: BMI2, CMOV
TEXT ·mulSubVecToX86(SB), NOSPLIT, $8-96
	MOVQ q+48(FP), AX
	MOVQ divHi+56(FP), CX
	MOVQ divLo+64(FP), BX
	MOVQ v0_base+0(FP), SI
	MOVQ v1_base+24(FP), DI
	MOVQ vOut_base+72(FP), R8
	MOVQ vOut_len+80(FP), R9
	XORQ R10, R10
	JMP  loop_end

loop_body:
	MOVQ    (SI)(R10*8), DX
	MOVQ    (DI)(R10*8), R11
	MOVQ    (R8)(R10*8), R12
	MULXQ   R11, R13, R11
	MOVQ    R11, R14
	IMULQ   CX, R14
	MOVQ    R13, DX
	MULXQ   CX, BP, R15
	ADDQ    R15, R14
	MULXQ   BX, DX, R15
	MOVQ    R11, DX
	MULXQ   BX, R11, DX
	ADDQ    DX, R14
	ADDQ    BP, R15
	ADCQ    $0x00, R14
	ADDQ    R11, R15
	ADCQ    $0x00, R14
	IMULQ   AX, R14
	MOVQ    R13, DX
	SUBQ    R14, DX
	MOVQ    DX, R11
	SUBQ    AX, R11
	CMPQ    DX, AX
	CMOVQGE R11, DX
	SUBQ    DX, R12
	MOVQ    R12, DX
	ADDQ    AX, DX
	CMPQ    R12, $0x00
	CMOVQLT DX, R12
	MOVQ    R12, (R8)(R10*8)
	ADDQ    $0x01, R10

loop_end:
	CMPQ R10, R9
	JL   loop_body
	RET

// func mulLazyVecToX86(v0 []uint64, v1 []uint64, q uint64, divHi uint64, divLo uint64, vOut []uint64)
// Requires: BMI2
TEXT ·mulLazyVecToX86(SB), NOSPLIT, $0-96
	MOVQ q+48(FP), AX
	MOVQ divHi+56(FP), CX
	MOVQ divLo+64(FP), BX
	MOVQ v0_base+0(FP), SI
	MOVQ v1_base+24(FP), DI
	MOVQ vOut_base+72(FP), R8
	MOVQ vOut_len+80(FP), R9
	XORQ R10, R10
	JMP  loop_end

loop_body:
	MOVQ  (SI)(R10*8), DX
	MOVQ  (DI)(R10*8), R11
	MULXQ R11, R12, R11
	MOVQ  R11, R13
	IMULQ CX, R13
	MOVQ  R12, DX
	MULXQ CX, R15, R14
	ADDQ  R14, R13
	MULXQ BX, DX, R14
	MOVQ  R11, DX
	MULXQ BX, R11, DX
	ADDQ  DX, R13
	ADDQ  R15, R14
	ADCQ  $0x00, R13
	ADDQ  R11, R14
	ADCQ  $0x00, R13
	IMULQ AX, R13
	MOVQ  R12, DX
	SUBQ  R13, DX
	MOVQ  DX, (R8)(R10*8)
	ADDQ  $0x01, R10

loop_end:
	CMPQ R10, R9
	JL   loop_body
	RET

// func mulAddLazyVecToX86(v0 []uint64, v1 []uint64, q uint64, divHi uint64, divLo uint64, vOut []uint64)
// Requires: BMI2
TEXT ·mulAddLazyVecToX86(SB), NOSPLIT, $8-96
	MOVQ q+48(FP), AX
	MOVQ divHi+56(FP), CX
	MOVQ divLo+64(FP), BX
	MOVQ v0_base+0(FP), SI
	MOVQ v1_base+24(FP), DI
	MOVQ vOut_base+72(FP), R8
	MOVQ vOut_len+80(FP), R9
	XORQ R10, R10
	JMP  loop_end

loop_body:
	MOVQ  (SI)(R10*8), DX
	MOVQ  (DI)(R10*8), R11
	MOVQ  (R8)(R10*8), R12
	MULXQ R11, R13, R11
	MOVQ  R11, R14
	IMULQ CX, R14
	MOVQ  R13, DX
	MULXQ CX, BP, R15
	ADDQ  R15, R14
	MULXQ BX, DX, R15
	MOVQ  R11, DX
	MULXQ BX, R11, DX
	ADDQ  DX, R14
	ADDQ  BP, R15
	ADCQ  $0x00, R14
	ADDQ  R11, R15
	ADCQ  $0x00, R14
	IMULQ AX, R14
	MOVQ  R13, DX
	SUBQ  R14, DX
	ADDQ  DX, R12
	MOVQ  R12, (R8)(R10*8)
	ADDQ  $0x01, R10

loop_end:
	CMPQ R10, R9
	JL   loop_body
	RET

// func mulSubLazyVecToX86(v0 []uint64, v1 []uint64, q uint64, divHi uint64, divLo uint64, vOut []uint64)
// Requires: BMI2
TEXT ·mulSubLazyVecToX86(SB), NOSPLIT, $8-96
	MOVQ q+48(FP), AX
	MOVQ divHi+56(FP), CX
	MOVQ divLo+64(FP), BX
	MOVQ v0_base+0(FP), SI
	MOVQ v1_base+24(FP), DI
	MOVQ vOut_base+72(FP), R8
	MOVQ vOut_len+80(FP), R9
	XORQ R10, R10
	JMP  loop_end

loop_body:
	MOVQ  (SI)(R10*8), DX
	MOVQ  (DI)(R10*8), R11
	NEGQ  DX
	ADDQ  AX, DX
	MOVQ  (R8)(R10*8), R12
	MULXQ R11, R13, R11
	MOVQ  R11, R14
	IMULQ CX, R14
	MOVQ  R13, DX
	MULXQ CX, BP, R15
	ADDQ  R15, R14
	MULXQ BX, DX, R15
	MOVQ  R11, DX
	MULXQ BX, R11, DX
	ADDQ  DX, R14
	ADDQ  BP, R15
	ADCQ  $0x00, R14
	ADDQ  R11, R15
	ADCQ  $0x00, R14
	IMULQ AX, R14
	MOVQ  R13, DX
	SUBQ  R14, DX
	ADDQ  DX, R12
	MOVQ  R12, (R8)(R10*8)
	ADDQ  $0x01, R10

loop_end:
	CMPQ R10, R9
	JL   loop_body
	RET
