...
// ScalarMulVecTo computes vOut = v0 * c mod q using Barrett reduction.
func ScalarMulVecTo(v0 []uint64, c uint64, q *Modulus, vOut []uint64) {
	if cpu.X86.HasBMI2 {
		divHi, divLo := q.Div()
		scalarMulVecToX86(v0, c, q.Value(), divHi, divLo, vOut)
		return
	}
	...
	for i := 0; i < M; i += 8 {
		...
		wOut[0] = bMul(w0[0], c, qv, divHi, divLo)
		wOut[1] = bMul(w0[1], c, qv, divHi, divLo)
		wOut[2] = bMul(w0[2], c, qv, divHi, divLo)
		wOut[3] = bMul(w0[3], c, qv, divHi, divLo)

		wOut[4] = bMul(w0[4], c, qv, divHi, divLo)
		wOut[5] = bMul(w0[5], c, qv, divHi, divLo)
		wOut[6] = bMul(w0[6], c, qv, divHi, divLo)
		wOut[7] = bMul(w0[7], c, qv, divHi, divLo)
	}

	for i := M; i < len(vOut); i++ {
		vOut[i] = bMul(v0[i], c, qv, divHi, divLo)
	}
}

// ScalarMulAddVecTo computes vOut += v0 * c mod q using Barrett reduction.
func ScalarMulAddVecTo(v0 []uint64, c uint64, q *Modulus, vOut []uint64) {
	if cpu.X86.HasBMI2 {
		divHi, divLo := q.Div()
		scalarMulVecAddToX86(v0, c, q.Value(), divHi, divLo, vOut)
		return
	}
	...
	for i := 0; i < M; i += 8 {
		...
		wOut[0] = add(wOut[0], bMul(w0[0], c, qv, divHi, divLo), qv)
		wOut[1] = add(wOut[1], bMul(w0[1], c, qv, divHi, divLo), qv)
		wOut[2] = add(wOut[2], bMul(w0[2], c, qv, divHi, divLo), qv)
		wOut[3] = add(wOut[3], bMul(w0[3], c, qv, divHi, divLo), qv)

		wOut[4] = add(wOut[4], bMul(w0[4], c, qv, divHi, divLo), qv)
		wOut[5] = add(wOut[5], bMul(w0[5], c, qv, divHi, divLo), qv)
		wOut[6] = add(wOut[6], bMul(w0[6], c, qv, divHi, divLo), qv)
		wOut[7] = add(wOut[7], bMul(w0[7], c, qv, divHi, divLo), qv)
	}

	for i := M; i < len(vOut); i++ {
		vOut[i] = add(vOut[i], bMul(v0[i], c, qv, divHi, divLo), qv)
	}
}

// ScalarMulSubVecTo computes vOut -= v0 * c mod q using Barrett reduction.
func ScalarMulSubVecTo(v0 []uint64, c uint64, q *Modulus, vOut []uint64) {
	if cpu.X86.HasBMI2 {
		divHi, divLo := q.Div()
		scalarMulVecSubToX86(v0, c, q.Value(), divHi, divLo, vOut)
		return
	}
	...
	for i := 0; i < M; i += 8 {
		...
		wOut[0] = sub(wOut[0], bMul(w0[0], c, qv, divHi, divLo), qv)
		wOut[1] = sub(wOut[1], bMul(w0[1], c, qv, divHi, divLo), qv)
		wOut[2] = sub(wOut[2], bMul(w0[2], c, qv, divHi, divLo), qv)
		wOut[3] = sub(wOut[3], bMul(w0[3], c, qv, divHi, divLo), qv)

		wOut[4] = sub(wOut[4], bMul(w0[4], c, qv, divHi, divLo), qv)
		wOut[5] = sub(wOut[5], bMul(w0[5], c, qv, divHi, divLo), qv)
		wOut[6] = sub(wOut[6], bMul(w0[6], c, qv, divHi, divLo), qv)
		wOut[7] = sub(wOut[7], bMul(w0[7], c, qv, divHi, divLo), qv)
	}

	for i := M; i < len(vOut); i++ {
		vOut[i] = sub(vOut[i], bMul(v0[i], c, qv, divHi, divLo), qv)
	}
}

// ScalarMulLazyVecTo computes vOut = v0 * c mod q using Barrett reduction,
// but the result is in [0, 2q).
func ScalarMulLazyVecTo(v0 []uint64, c uint64, q *Modulus, vOut []uint64) {
	if cpu.X86.HasBMI2 {
		divHi, divLo := q.Div()
		scalarMulLazyVecToX86(v0, c, q.Value(), divHi, divLo, vOut)
		return
	}
	...
	for i := 0; i < M; i += 8 {
		...
		wOut[0] = bMulLazy(w0[0], c, qv, divHi, divLo)
		wOut[1] = bMulLazy(w0[1], c, qv, divHi, divLo)
		wOut[2] = bMulLazy(w0[2], c, qv, divHi, divLo)
		wOut[3] = bMulLazy(w0[3], c, qv, divHi, divLo)

		wOut[4] = bMulLazy(w0[4], c, qv, divHi, divLo)
		wOut[5] = bMulLazy(w0[5], c, qv, divHi, divLo)
		wOut[6] = bMulLazy(w0[6], c, qv, divHi, divLo)
		wOut[7] = bMulLazy(w0[7], c, qv, divHi, divLo)
	}

	for i := M; i < len(vOut); i++ {
		vOut[i] = bMulLazy(v0[i], c, qv, divHi, divLo)
	}
}

// ScalarMulAddLazyVecTo computes vOut += v0 * c mod q using Barrett reduction,
// but the result is in [0, 2q).
func ScalarMulAddLazyVecTo(v0 []uint64, c uint64, q *Modulus, vOut []uint64) {
	if cpu.X86.HasBMI2 {
		divHi, divLo := q.Div()
		scalarMulAddLazyVecToX86(v0, c, q.Value(), divHi, divLo, vOut)
		return
	}
...
// ScalarMulSubLazyVecTo computes vOut -= v0 * c mod q using Barrett reduction,
// but the result is in [0, 2q).
func ScalarMulSubLazyVecTo(v0 []uint64, c uint64, q *Modulus, vOut []uint64) {
	if cpu.X86.HasBMI2 {
		divHi, divLo := q.Div()
		scalarMulSubLazyVecToX86(v0, c, q.Value(), divHi, divLo, vOut)
		return
	}
	...
	for i := 0; i < M; i += 8 {
		...
		wOut[0] += bMulLazy(w0[0], cNeg, qv, divHi, divLo)
		wOut[1] += bMulLazy(w0[1], cNeg, qv, divHi, divLo)
		wOut[2] += bMulLazy(w0[2], cNeg, qv, divHi, divLo)
		wOut[3] += bMulLazy(w0[3], cNeg, qv, divHi, divLo)

		wOut[4] += bMulLazy(w0[4], cNeg, qv, divHi, divLo)
		wOut[5] += bMulLazy(w0[5], cNeg, qv, divHi, divLo)
		wOut[6] += bMulLazy(w0[6], cNeg, qv, divHi, divLo)
		wOut[7] += bMulLazy(w0[7], cNeg, qv, divHi, divLo)
	}

	for i := M; i < len(vOut); i++ {
		vOut[i] += bMulLazy(v0[i], cNeg, qv, divHi, divLo)
	}
}
