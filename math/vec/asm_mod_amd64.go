...
func subVecScalarTo(vOut, v []uint64, c uint64, q *num.Modulus) {
	...
	switch {
	case cpu.X86.HasAVX512F:
		subVecScalarToAVX512(vOut, v, c, q.Value())
		return
	case cpu.X86.HasAVX && cpu.X86.HasAVX2:
		subVecScalarToAVX2(vOut, v, c, q.Value())
		return
	}
...
func subScalarVecTo(vOut []uint64, c uint64, v []uint64, q *num.Modulus) {
	...
	switch {
	case cpu.X86.HasAVX512F:
		subScalarVecToAVX512(vOut, c, v, q.Value())
		return
	case cpu.X86.HasAVX && cpu.X86.HasAVX2:
		subScalarVecToAVX2(vOut, c, v, q.Value())
		return
	}
...
func subVecScalarWordTo(vOut, v []uint64, c uint64) {
	...
	switch {
	case cpu.X86.HasAVX512F:
		subVecScalarWordToAVX512(vOut, v, c)
		return
	case cpu.X86.HasAVX && cpu.X86.HasAVX2:
		subVecScalarWordToAVX2(vOut, v, c)
		return
	}
...
func subScalarVecWordTo(vOut []uint64, c uint64, v []uint64) {
	...
	switch {
	case cpu.X86.HasAVX512F:
		subScalarVecWordToAVX512(vOut, c, v)
		return
	case cpu.X86.HasAVX && cpu.X86.HasAVX2:
		subScalarVecWordToAVX2(vOut, c, v)
		return
	}
...
func mMulSubScalarLazyTo(vOutM, vM []uint64, cM uint64, q *num.Modulus) {
	...
	case cpu.X86.HasAVX512DQ && cpu.X86.HasAVX512F && cpu.X86.HasBMI2:
		qv := q.Value()
		mMulSubScalarLazyToAVX512(vOutM, vM, modops.Sub(0, cM, qv), qv, q.Inv())
		return
	}
