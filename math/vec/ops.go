...
func MMulSubLazyTo(v0M, v1M []uint64, q *num.Modulus, vOutM []uint64) {
	...
	for i := 0; i < M; i += 8 {
		...
		wOut[0] += qv<<1 - mod.MMulLazy(w0[0], w1[0], qv, inv)
		wOut[1] += qv<<1 - mod.MMulLazy(w0[1], w1[1], qv, inv)
		wOut[2] += qv<<1 - mod.MMulLazy(w0[2], w1[2], qv, inv)
		wOut[3] += qv<<1 - mod.MMulLazy(w0[3], w1[3], qv, inv)

		wOut[4] += qv<<1 - mod.MMulLazy(w0[4], w1[4], qv, inv)
		wOut[5] += qv<<1 - mod.MMulLazy(w0[5], w1[5], qv, inv)
		wOut[6] += qv<<1 - mod.MMulLazy(w0[6], w1[6], qv, inv)
		wOut[7] += qv<<1 - mod.MMulLazy(w0[7], w1[7], qv, inv)
	}

	for i := M; i < len(vOutM); i++ {
		vOutM[i] += qv<<1 - mod.MMulLazy(v0M[i], v1M[i], qv, inv)
	}
}
