...
// NewCyclotomicReducer creates a new [Reducer] for cyclotomic polynomials.
func NewCyclotomicReducer(params RingParameters, modulus *mod.Modulus) *Reducer {
	return &Reducer{
		reducer: newCyclotomicReducer(params, modulus),
	}
}

// NewReducer creates a new [Reducer] for arbitrary polynomials.
func NewReducer(maxDeg int, modPoly []uint64, modulus *mod.Modulus) *Reducer {
	return &Reducer{
		reducer: newReducer(maxDeg, modPoly, modulus),
	}
}
...
// newCyclotomicReducer creates a [reducer] specific for cyclotomic polynomials.
func newCyclotomicReducer(params RingParameters, modulus *mod.Modulus) reducer {
	if isNTTFriendly(params, modulus) {
		return newCyclotomicReducerNTTModulus(params, modulus)
	}
	return newCyclotomicReducerAnyModulus(params, modulus)
}
...
func newReducer(maxDeg int, modPoly []uint64, modulus *mod.Modulus) reducer {
	...
	if isNTTFriendly(NewCyclicParameters(max(degNext, diffDegNext)), modulus) {
		return newReducerNTTModulus(maxDeg, modPoly, modulus)
	}
	return newReducerAnyModulus(maxDeg, modPoly, modulus)
}
